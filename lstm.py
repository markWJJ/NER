...
tf.app.flags.DEFINE_integer("Q_len", config.sent_len, "句子长度")
...
class Bilstm(object):
    ...
    def __init__(self,num_class,hidden_dim,seq_len,seq_dim,mode,batch_size,crf_mode,loss_weight_mode):
        ...
        self.X_sent=tf.placeholder(shape=(None,self.seq_len,self.seq_dim),dtype=tf.float32)
        #self.X_entity=tf.placeholder(shape=(None,self.seq_len,self.seq_dim),dtype=tf.float32)
        self.Y = tf.placeholder(shape=(None,self.seq_len), dtype=tf.int32)
        #self.keep_dropout=tf.placeholder(dtype=tf.float32)
        #self.X=tf.concat([self.X_sent,self.X_entity],2)
        self.X=self.X_sent
        X_=tf.transpose(self.X,[1,0,2])
        ...
        with tf.device("/cpu:4"):#4
            ...
            elif self.mode == "lstm_attention_crf":
                pass
    ...
    def crf_train(self,dd):
            ...
            self.crf_mode="train"
            with tf.Session(config=config) as sess:
                #saver.restore(sess,"./save_model/lstm_crf_model.ckpt")
                #sess.run(tf.global_variables_initializer())
                ...
                init_dev_acc=0.0
                for i in range(5000):
                    sent, entity, train_label,loss_weight,train_real_len=dd.next_batch()
                    '''
                    ...
                    verbit_seq=self.Verbit(logits=logit,batch_size=sent.shape[0],trans_params=trans_params,sequence_lengths=seq_vec)
                    ...
                    crf_acc_train=self.crf_acc(verbit_seq,train_label,train_real_len)
                    ...
                    if crf_acc_train>init_train_acc and crf_acc_dev >init_dev_acc:
                        print("save model")
                        saver.save(sess,"./save_model/lstm_crf_new1.model")
...
if __name__ == '__main__':
    ...
    with tf.device("/cpu:0"):
        ...
        dd = Data_deal.DataDeal(train_path="./data/data1.txt", test_path="./data/test.txt",
        ...
        nn_model = Bilstm(hidden_dim=FLAGS.hidden_dim,
                      ...
                      num_class=num_class,
                      mode="lstm+crf",batch_size=FLAGS.batch_size)
