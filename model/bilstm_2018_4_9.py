...
class Bilstm(object):
    ...
    def __init__(self,num_class,hidden_dim,seq_dim,mode,batch_size,embedding_dim,vocab_num,crf_mode,
        ...
        with tf.device("/cpu:4"):#4
            ...
            if self.mode == "bilstm":
                ...
                self.loss_op=tf.losses.softmax_cross_entropy(YY,self.lstm_prediction)
                ...
                tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(FLAGS.learning_rate)(lstm_w))
                logit = tf.add(tf.einsum("ijk,kl->ijl", lstm_out, lstm_w), lstm_b)
                ...
                lstm_out=tf.nn.dropout(lstm_out,1.0)
                tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(0.01)(lstm_w))
                logit = tf.add(tf.einsum("ijk,kl->ijl", lstm_out, lstm_w), lstm_b)
                #label = tf.one_hot(self.Y, self.num_class, 1, 0,2)
                ...
                logit=self.cnn_layer(input_emb)
                log_likelihood, trans_params = tf.contrib.crf.crf_log_likelihood(
                    logit, self.Y, self.seq_vec)
                self.logit = logit
                self.trans_params = trans_params  # need to evaluate it for decoding
    ...
    def cnn_layer(self,input_emb):
        ...
        filter_w=tf.Variable(tf.random_uniform(shape=(1,self.hidden_dim,1,self.hidden_dim),dtype=tf.float32))
        conv1=tf.nn.conv2d(cnn_input,filter_w,strides=[1,1,1,1],padding="SAME")
        filter_w_1=tf.Variable(tf.random_uniform(shape=(1,self.hidden_dim,self.hidden_dim,2*self.hidden_dim),dtype=tf.float32))
        conv2=tf.nn.conv2d(conv1,filter_w_1,strides=[1,1,1,1],padding="VALID")
        conv_out=tf.reshape(conv2,[-1,self.seq_len,2*self.hidden_dim])

        softmax_w=tf.Variable(tf.random_uniform(shape=(2*self.hidden_dim,self.num_class),dtype=tf.float32))
        softmax_b=tf.Variable(tf.random_uniform(shape=(self.num_class,),dtype=tf.float32))
        conv_out=tf.einsum('ijk,kl->ijl',conv_out,softmax_w)
        conv_out=tf.add(conv_out,softmax_b)
        conv_out=tf.nn.softmax(conv_out,2)
        return conv_out
